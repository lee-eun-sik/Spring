...
<head> 
	...
	$(document).ready(function() {
		...
		ajaxRequest(
				"/reviewboard/reviewviewCount.do",
				{
		...
		$("#updateBtn").click( function(event) {
			...
			if($("#createId").val() == '${sessionScope.user.userId}'){
				window.location.href = "/reviewboard/reviewupdate.do?id=${board.boardId}";
			} else {
				alert("아이디가 일치하지않음.")
			}
...
<h2>상세 후기</h2>
	...
	<c:choose>
	    <c:when test="${board.rating == 1}">⭐</c:when>
	    <c:when test="${board.rating == 2}">⭐⭐</c:when>
	    <c:when test="${board.rating == 3}">⭐⭐⭐</c:when>
	    <c:when test="${board.rating == 4}">⭐⭐⭐⭐</c:when>
	    <c:when test="${board.rating == 5}">⭐⭐⭐⭐⭐</c:when>
	    <c:otherwise>
	        별점 없음
	    </c:otherwise>
	</c:choose>
		<br/><br/>
	  <label for="createId">펫시터 : ${board.sitter}</label> 
	  <br/>
	...
	<c:if test="${not empty sessionScope.user.userId}">
		<h4>댓글 작성</h4>
		<textarea id="commentContent" rows="4" placeholder="댓글을 입력해주세요"></textarea>
		<button type="button" id="commentCreateBtn" onclick="addComment()">댓글작성</button>
	</c:if>
